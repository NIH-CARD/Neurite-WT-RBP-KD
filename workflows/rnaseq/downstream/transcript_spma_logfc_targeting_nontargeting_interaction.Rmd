...
guides <- c(targeting_guide, nontargeting_guide)
dt_colData_sub <- dt_colData[(group %in% target),]
dt_colData_sub <- dt_colData_sub[guide_group %in% guides,]

# If we're dealing with  sg200_N_TDP43kd_13_B2 get rid of it
# for the interaction analysis
dt_colData_sub <- subset(dt_colData_sub, sample != 'sg200_N_TDP43kd_13_B2')

# Set conditions for this round
dt_colData_sub$condition <- factor(dt_colData_sub$guide_group ,levels=c(targeting_guide,nontargeting_guide))
...
y <- swish(y, x="zone", cov='condition', pair="pair", interaction=TRUE)
