...
contrast_name <- 'TDP43_N_sg200_vs_sg100'
...
p <- ggplot(data=mat,aes_string(x = first_PC, y = second_PC, color = color_var)) +
    ...
    geom_point(size=3) + xlab(paste0(first_PC,': ', round(pv[1]*100), '% variance')) +
          ...
          ggtitle(paste0(first_PC, " and ", second_PC, " TDP 43 samples by cellular zone"))
ggsave(paste0("Publication_PCA_plots/", variable, "figure_4_pca_",first_PC, "_", second_PC, "_TD43_KD.pdf"), plot=p)
```
