...
#contrast_name <- 'TDP43_N_targeting_over_nontargeting'
...
```{r dds_initial, cache=FALSE, config=c(config$main, config$toggle$salmon, config$toggle$kallisto), dependson=c('config', 'coldata_setup')}
...
```{r test}
my_function <- function(ya, dt_colData_WT) {
...
```{r PCA_gene_level,config=c(config$main, config$toggle$salmon, config$toggle$kallisto)}
...
lcdbwf:::createSwishPCA(yg, PC1, PC2, variable, contrast_name, extra_info)
```

```{r PCA,config=c(config$main, config$toggle$salmon, config$toggle$kallisto)}
extra_info <- "transcript"

lcdbwf:::createSwishPCA(y, PC1, PC2, variable, contrast_name, extra_info)
```


```{r fishpond_gene_level, cache=FALSE, dependson=c('dds_initial', 'coldata_setup')}
library(fishpond)
library(tibble)
yg <- scaleInfReps(yg)
yg <- labelKeep(yg)
...
df_gene <- mcols(yg)
df_gene$tx_name <- rownames(df_gene)
sign_tran_gene <- as.tibble(df_gene)
sign_tran_gene <- sign_tran_gene[sign_tran_gene$qvalue < 0.05,]
# Must arrange by something for transite. Must specify what that is. They suggest log2FC
sign_tran_gene <- dplyr::arrange(sign_tran_gene, log2FC, locfdr)
sign_out_gene <- apply(sign_tran_gene, 2, as.character)
write.csv(sign_out_gene, paste0(base_folder, "diff_gene_figure_4_", contrast_name, ".csv"), quote=FALSE, row.names=FALSE)
```


```{r fishpond_transcript_level, cache=FALSE, dependson=c('dds_initial', 'coldata_setup')}
library(fishpond)
library(tibble)
y <- scaleInfReps(y)
y <- labelKeep(y)
...
rows <- nrow(sign_tran_gene)

# delete original file if already there
transcripts_file <- paste0(base_folder, "transcripts_", contrast_name, "_gene_level.csv")
...
write.table(sequences_tei_1, paste0(base_folder, "transcripts_",contrast_name,"_gene_level.csv"), quote=FALSE, sep=',', row.names=FALSE)
...
```{r maplot,cache=FALSE,config=c(config$main, config$toggle$salmon, config$toggle$kallisto), dependson=c('swish', 'tximeta_setup','dds_initial')}
library(ggpubr)
library(tidyverse)
...
library(purrr)
library(stringr)
gene_ids_swee <- mcols(yg)$gene_id
...
relevant_data <- as.data.table(ma_plot_data[ma_plot_data$padj < 0.05,])
relevant_data <- relevant_data[relevant_data$log2FoldChange > 0]
relevant_data <- relevant_data[order(padj, -log2FoldChange)]
relevant_data_names <- relevant_data[,'ENSEMBL']
write.csv(relevant_data_names, paste0("final_figures/diff_f0r_GO", contrast_name, ".csv"), quote=FALSE, row.names=FALSE)
```
