...
dt_colData_WT$cellular_zone <- factor(dt_colData_WT$full_guide_grp ,levels=c(soma_contrast, neurite_contrast))
```
...
ggsave(paste0("Publication_PCA_plots/", variable, "figure_2_pca_",first_PC, "_", second_PC, "_only_WT.pdf"), plot=p)


```

```{r swish, cache=TRUE, config=c(config$main, config$toggle$salmon, config$toggle$kallisto), dependson=c('PCA', 'tximeta_setup', 'dds_initial')}
...
# Create volcano plot
# Because of the way swish does statistics a weird volcano plot is expected
# Use an MA plot instead, it has more information anyway
...
ggmaplot(ma_plot_data,  main = expression("S3 Neurite / S3 Soma"), 
    fc=1.5, 
    fdr=0.05, 
