...
config <- lcdbwf:::load_config('config_gsea.yaml')
...
removeHomo <- function(x) {
  ...
  x@result$Description <- gsub("^GOMF", "", x@result$Description)
  x@result$Description <- gsub("^GOBP", "", x@result$Description)
  x@result$Description <- gsub("^GOBP", "", x@result$Description)
  x@result$Description <- gsub("^GOCC", "", x@result$Description)
...
```{r, results='asis'}
path_initial <- "functional-enrichment_files/gsea/"
    ...
    for (ont in names(enrich_list[[name]][[direction]])){
      ...
      lcdbwf:::mdcat("##### dotplot")
        if (length(dotplot_list[[name]][[direction]][[ont]]) < 1){
            ...
            savename <- paste(name, direction, ont, "dotplot_gsea", sep="_")
